...
const Connect: React.FC<{ text?: string }> = ({ text = "Let's Connect" }) => {
  const [showForm, setShowForm] = useState(false);

  const handlePopup = () => setShowForm(true);
  const closePopup = () => setShowForm(false);

  return (
    <StyledWrapper>
      {/* Trigger button */}
      <button className="cta" onClick={handlePopup} aria-label={text}>
        <span>{text}</span>
      ...
      {showForm && (
        <div
          ...
          onClick={closePopup}          /* clicking outside closes */
        >
          <div onClick={e => e.stopPropagation()}>
            <ContactUS closeModal={closePopup} />
          </div>
        </div>
      )}
    </StyledWrapper>
...
const StyledWrapper = styled.div`
  .cta {
    ...
    min-width: 140px; /* Ensure good clickable area */
  }
  ...
  .cta span {
    ...
    font-size: clamp(16px, 2vw, 18px); /* Responsive text */
    font-weight: 700;
  ...
  .cta:hover:before {
    width: 100%;
    background: rgb(89, 117, 126);
  }

  .cta:hover svg {
    transform: translateX(2px); /* Smoother arrow animation */
    stroke: #fff;
  }

  .cta:hover span {
    color: #e0f4ff; /* Text color change on hover */
  }
